...
def plot_attention():
    ...
    attention_weights = np.load('mains/saved_attention_weights.npy', allow_pickle=True)
    ...
    for i, (patches, labels) in enumerate(dataset_test):
        ...
        plt.plot(patches[max_attention_patch], label='bag {}'.format(i+1))
        plt.title('Max attention batches')
        plt.savefig('images/max_attention_batches.pdf')
        '''
        whole_img = np.concatenate(patches, axis=1)
